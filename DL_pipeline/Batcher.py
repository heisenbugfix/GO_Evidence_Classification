...
class Batcher(object):
    def __init__(self, config, abstracts_file, data_file, vocab, batch_size = 100, return_one_epoch=False, shuffle=True, triplet=True):
        ...
        with codecs.open(filepath, "r", "UTF-8") as rf:
            ...
            for key in keys:
                ...
                if len(doc) >= self.config.max_abstract_len:
                    ...
                    while len(doc) < max_len and len(doc) < self.config.max_abstract_len:
                        doc.append(self.vocab_length)
    ...
    def get_next_batch(self):
        ...
        while True:
            ...
            if self.start_index > self.num_examples - self.batch_size:
                ...
                yield self.gene_ids[self.start_index:end_index], self.abstract_encodings[self.start_index:end_index], self.abstract_lengths_list[self.start_index:end_index], self.evidence_labels[self.start_index:end_index], self.aspects[self.start_index:end_index]
    ...
    def shuffle_data(self):
        ...
        """
        perm = np.random.permutation(self.gene_ids)
        assert len(perm) == self.gene_ids
        ...
        self.abstract_lengths_list = self.abstract_lengths_list[perm]
        ...
        with codecs.open(filepath, "r", "UTF-8") as rf:
            for line in rf:
                ...
                if split[5].split(":")[0] == "PMID":
                    ...
                    gene_ids.append(split[4].split(":")[1])
    ...
    def evidence_codes_to_onehot(self, evidence_codes):
        ...
        EV_TO_ONEHOT_DICT["IDA"] = [0] * 1 + [1] + [0] * 18
        EV_TO_ONEHOT_DICT["IPI"] = [0] * 2 + [1] + [0] * 17
        EV_TO_ONEHOT_DICT["IMP"] = [0] * 3 + [1] + [0] * 16
        EV_TO_ONEHOT_DICT["IGI"] = [0] * 4 + [1] + [0] * 15
        EV_TO_ONEHOT_DICT["IEP"] = [0] * 5 + [1] + [0] * 14
        EV_TO_ONEHOT_DICT["ISS"] = [0] * 6 + [1] + [0] * 13
        EV_TO_ONEHOT_DICT["ISO"] = [0] * 7 + [1] + [0] * 12
        EV_TO_ONEHOT_DICT["ISA"] = [0] * 8 + [1] + [0] * 11
        EV_TO_ONEHOT_DICT["ISM"] = [0] * 9 + [1] + [0] * 10
        EV_TO_ONEHOT_DICT["ISC"] = [0] * 10 + [1] + [0] * 9
        EV_TO_ONEHOT_DICT["IBA"] = [0] * 11 + [1] + [0] * 8
        EV_TO_ONEHOT_DICT["IBD"] = [0] * 12 + [1] + [0] * 7
        EV_TO_ONEHOT_DICT["IKR"] = [0] * 13 + [1] + [0] * 6
        EV_TO_ONEHOT_DICT["IRD"] = [0] * 14 + [1] + [0] * 5
        EV_TO_ONEHOT_DICT["RCA"] = [0] * 15 + [1] + [0] * 4
        EV_TO_ONEHOT_DICT["TAS"] = [0] * 16 + [1] + [0] * 3
        EV_TO_ONEHOT_DICT["NAS"] = [0] * 17 + [1] + [0] * 2
        EV_TO_ONEHOT_DICT["IC"] = [0] * 18 + [1] + [0] * 1
        EV_TO_ONEHOT_DICT["ND"] = [0] * 19 + [1]
    ...
    def load_data(self, abstracts_file, data_file):
        ...
        self.abstract_lengths_list = np.asarray([self.abstract_lengths[i] for i in self.gene_pmids])
    ...
    def tokenize(self, abstract):
        ...
        tokenized_to_vocab = [self.vocab.get(i, self.vocab["OOV"]) for i in tokenized]
        return tokenized_to_vocab
