...
class model_evaluation:
    ...
    def compute_accuracy_score(self, y_true, y_pred):
        ...
        pred_sum = np.sum(val, axis=1)
        acc_per_point = pred_sum/true_sum
        avg_acc = np.mean(acc_per_point)
        # print(avg_acc)
        return avg_acc
        ############################
...
# yt = np.asarray([[0,1,1],[0,1,1]])
# yp = np.asarray([[0,1,0],[1,0,0]])
# m = model_evaluation(yt)
# m.compute_accuracy_score(yt, yp)
'''
